...
use App\Model\Admin\ActivityCategory as ThisModel;
...
{
    protected $view = 'admin.activity_categories';
    protected $route = 'ActivityCategory';
    ...
    public function nestedSort(Request $request)
    {
        if ($request->ajax()) {
            ...
            foreach($readbleArray as $row){
                ...
                $i++;
                DB::table('activity_categories')->where('id',$row['id'])->update(['parent_id' => $row['parentID'], 'sort_order' => $i, 'level' => $level]);
            }
            $json->success = true;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($request->image) {
                FileHelper::uploadFile($request->image, 'activity_categories', $object->id, ThisModel::class, 'image',1);
            }
            DB::commit();
            $json->success = true;
